...
export class ClientsService {
  ...
  async sendSms(clientId: string, text: string) {
    const client = await this.clientsRepo.findOne({ where: { id: clientId } });
    if (!client) throw new NotFoundException('Клиент не найден');
    const message = String(text || '').trim();
    if (!message) {
      throw new BadRequestException('Текст SMS не должен быть пустым');
    }
    ...
    if (smsUrl && phone && message) {
      try {
        ...
        if (!res.ok) {
          await this.smsRepo.update(saved.id, {
            status: 'failed',
            error: `HTTP ${res.status}`,
          });
          return { success: false };
        }
        await this.smsRepo.update(saved.id, { status: 'sent' });
      } catch (e) {
        await this.smsRepo.update(saved.id, {
          status: 'failed',
      ...
      await this.smsRepo.update(saved.id, {
        status: phone && message ? 'sent' : 'failed',
        error: !smsUrl ? 'SMS_WEBHOOK_URL не настроен, записано в лог' : undefined,
      });
    }
    return { success: true };
  }
